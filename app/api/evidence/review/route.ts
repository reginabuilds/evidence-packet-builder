...
function proposalId(analysisId: string, proposalIndex: number) {
  return `${analysisId}:${proposalIndex}`;
}
...
export async function GET(request: Request) {
  try {
    ...
    const analysisQuery = admin
      .from("evidence_ai_analyses")
      .select("id, evidence_id, analysis_json, generator_name, generator_model, source_mode, created_at")
      .eq("owner_id", user.id)
      .order("created_at", { ascending: false });
    const { data: analysis, error: analysisError } = analysisId
      ? await analysisQuery.eq("id", analysisId).maybeSingle()
      : await analysisQuery.limit(1).maybeSingle();
    ...
    const { data: decisions, error: decisionsError } = await admin
      .from("evidence_ai_proposal_reviews")
      .select("id, analysis_id, evidence_id, proposal_index, proposal_id, student_id, decision, decided_at")
...
export async function POST(request: Request) {
  try {
    const { user, admin } = await authenticateApplicant(request);
    const body = await request.json().catch(() => null) as { analysisId?: unknown; proposalIndex?: unknown; decision?: unknown } | null;
    ...
    if (!/^[0-9a-f-]{36}$/i.test(analysisId) || proposalIndex < 0 || !decision) {
    ...
    const { data: analysis, error: analysisError } = await admin
      .from("evidence_ai_analyses")
      .select("id, evidence_id, owner_id, analysis_json")
      .eq("id", analysisId)
      .eq("owner_id", user.id)
      .maybeSingle();
    if (analysisError) throw new Error("Unable to load AI proposal.");
    ...
    if (!Array.isArray(proposals) || proposalIndex >= proposals.length || !proposals[proposalIndex]) {
      return NextResponse.json({ error: "AI proposal not found." }, { status: 404 });
    }

    const id = proposalId(analysis.id, proposalIndex);
    const { data: existing, error: existingError } = await admin
      .from("evidence_ai_proposal_reviews")
      .select("id, decision, student_id")
      .eq("analysis_id", analysis.id)
      .eq("proposal_index", proposalIndex)
      .maybeSingle();
    if (existingError) throw new Error("Unable to check existing student decision.");
    if (existing && existing.decision === decision) {
      return NextResponse.json({ error: "This proposal already has that student decision." }, { status: 409 });
    }
    ...
    if (existing) {
      const { data, error } = await admin
        .from("evidence_ai_proposal_reviews")
        .update({ decision, student_id: user.id, evidence_id: evidence.id, proposal_id: id, decided_at: new Date().toISOString() })
        .eq("id", existing.id)
        .eq("student_id", user.id)
        .select("id, analysis_id, evidence_id, proposal_index, proposal_id, student_id, decision, decided_at")
        .single();
      ...
      const { data, error } = await admin
        .from("evidence_ai_proposal_reviews")
        .insert({ analysis_id: analysis.id, evidence_id: evidence.id, proposal_index: proposalIndex, proposal_id: id, student_id: user.id, decision })
        .select("id, analysis_id, evidence_id, proposal_index, proposal_id, student_id, decision, decided_at")
        .single();
      if (error) {
        if (error.code === "23505") return NextResponse.json({ error: "This proposal already has a student decision." }, { status: 409 });
        throw new Error("Student decision could not be saved.");
      }
      saved = data;
    }
    ...
    const { error: auditError } = await admin.from("transformation_events").insert({
      ...
      reason: `Student review decision: ${decision}. This records the student's decision on an AI proposal and does not certify capability.`,
    });
